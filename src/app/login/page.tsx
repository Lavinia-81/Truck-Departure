  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (!ADMIN_EMAILS.includes(signedInUser.email || '')) {
        setError('Your email is not authorized for access.');
        await auth.signOut();
      } else {
        router.replace('/');
      }
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-100 dark:bg-gray-900 p-4">
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center">
            <div className="mx-auto bg-white p-2 rounded-md shadow-sm w-40 mb-4">
          ...
          <CardDescription>Sign in to manage the departure board.</CardDescription>
        </CardHeader>
